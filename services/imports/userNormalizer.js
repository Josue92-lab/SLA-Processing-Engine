 ...
 *   1. TIER 3 drop: malformed rows (empty email, invalid userType) are
 *      dropped with a warning attached.
 *   2. KEPT: a typed NormalizedUser record is returned. Sub-issues like
 *      an invalid timezone or unresolvable country do NOT drop the row;
 *      they null-out the specific field and emit a warning.
 *
 * The source export is already scoped to the relevant support-group
 * population and active users by the upstream HR/ServiceNow system.
 * No import-time active/status filtering is applied — all parsed rows
 * with valid identity (email + userType) are kept.
 *
 ...
 *   skipped: null|'missingEmail'|'invalidEmail'|'invalidUserType',
 *   warnings: string[]
 * }}
 */
 ...
 *   dropped: { missingEmail: number, invalidEmail: number, invalidUserType: number },
 *   warnings: string[]
 * }}
 */
export const normalizeAll = (rawRows, source, deps = {}) => {
    const records = [];
    const warnings = [];
    const dropped = { missingEmail: 0, invalidEmail: 0, invalidUserType: 0 };
